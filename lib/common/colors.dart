import 'package:flutter/material.dart';

class AppColors{

  //Background Color
  static const Color welcomeScreenBackGround = Color(0xFF2C53D2);
  static const Color loginScreenBackGround = Color(0xFFFFFFFF);


  //Text Color
  static const Color blackTextColor = const Color(0xFF000000);
  static const Color lightTextColor = const Color(0xFF666666);
  static const Color blueTextColor = const Color(0xFF2C53D2);
  static const Color whiteTextColor = const Color(0xFFFFFFFF);
  static const Color greenTextColor = const Color(0xFF1CBE8E);

  //Button Color
  static const Color buttonColor = const Color(0xFF2C53D2);
  static const Color whiteButtonColor = const Color(0xFFEFEFF4);
  static const Color unselectedBtn = const Color(0xFF666666);

  //Search Bar Background
  static const Color searchBarColor = const Color(0xFFEFEFFF);

  //Icon color
  static const Color settingIcon = Color(0xFF2C53D2);
  static const Color handymanIcon = Color(0xFFFF981F);
  static const Color nocrashIcon = Color(0xFFFCD433);
  static const Color errorIcon = Color(0xFFEC1B22);

  //HomePage Service Button Color
  static const Color homepageServiceBtn = Color(0xFFEFEFF4);
}